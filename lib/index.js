...
class PluginManagerGateway {
  ...
  list() {
    ...
    // 第一遍：收集每个非 group 行的注入服务与提供的服务名。
    const rows = []
    for (const entry of this.ctx.loader.entries()) {
      if (entry.options.group) continue
      const moduleName = entry.options.name
      const fiber = entry.fiber
      const injects = fiber && fiber.inject ? Object.keys(fiber.inject).sort() : []
      const serviceName = fiber && fiber.name ? String(fiber.name) : null
      rows.push({ entry, moduleName, injects, serviceName })
    }

    // 服务注册表：服务名 -> 提供它的模块名（来自 ctx.reflect.store，准确）；模块 -> 提供的服务集合。
    const providerIndex = new Map()
    const providesByModule = new Map()
    const reflectStore = this.ctx.reflect && this.ctx.reflect.store
    if (reflectStore) {
      for (const impl of Object.values(reflectStore)) {
        const serviceName = impl && impl.name
        const fiber = impl && impl.fiber
        const moduleName = fiber && fiber.entry ? fiber.entry.options.name : null
        if (!serviceName || !moduleName) continue
        if (!providerIndex.has(serviceName)) providerIndex.set(serviceName, moduleName)
        if (!providesByModule.has(moduleName)) providesByModule.set(moduleName, new Set())
        providesByModule.get(moduleName).add(serviceName)
      }
    }

    // 反向索引：服务名 -> 注入它的模块名集合。
    const injectorIndex = new Map()
    for (const r of rows) {
      for (const s of r.injects) {
        if (!injectorIndex.has(s)) injectorIndex.set(s, new Set())
        injectorIndex.get(s).add(r.moduleName)
      }
    }
    ...
    for (const r of rows) {
      ...
      const provided = providesByModule.get(moduleName)
      const depSet = new Set()
      if (provided) {
        for (const s of provided) {
          const injectors = injectorIndex.get(s)
          if (injectors) for (const n of injectors) if (n !== moduleName) depSet.add(n)
        }
      }
      const dependents = [...depSet].sort()
      entries.push({
        entryId: entry.id,
        moduleName,
        enabled,
    ...
    return {
      ...
      enabledCount,
      groups: groupList,
      providers: Object.fromEntries(providerIndex),
      entries,
    }
  }
