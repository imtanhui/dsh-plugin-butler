...
class PluginManagerGateway {
  ...
  list() {
    ...
    // 反向索引：服务名 -> 注入它的模块名集合；服务名 -> 提供它的模块名。
    ...
    for (const r of rows) {
      ...
      if (r.serviceName && !providerIndex.has(r.serviceName)) providerIndex.set(r.serviceName, r.moduleName)
    }
    ...
    for (const r of rows) {
      ...
      const dependents = r.serviceName && injectorIndex.has(r.serviceName)
        ? [...injectorIndex.get(r.serviceName)].filter((n) => n !== moduleName).sort()
        : []
      entries.push({
        entryId: entry.id,
        moduleName,
