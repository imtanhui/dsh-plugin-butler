...
class PluginManagerGateway {
  ...
  update(name) {
    const run = async () => {
      ...
      const runAdd = (spec) => new Promise((resolveResult) => {
        let output = ''
        let settled = false
        const finish = (ok) => { if (!settled) { settled = true; resolveResult({ ok, output }) } }
        // 直接走 pnpm（dsh plugin add 的底层，全局 PATH 更稳）；更新场景无需 bundle reconcile（包已在层栈）。
