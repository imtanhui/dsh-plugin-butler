/**
 * dsh-plugin-manager — 插件管家 Host 半。
 *
...
import { githubUrl, initialPatchFile, moduleShortName, setRowDisabled } from './patch.js'
...
/** 按行 id 保护（与模块名无关的引导行）。 */
export const SYSTEM_ROW_IDS = new Set(['include', 'loader', 'plugin-manager'])
...
class PluginManagerGateway {
  ...
  /** 市场：搜索 npm registry（默认按 dsh-plugin 关键词）。 */
  market(query) {
    return (async () => {
      try {
        const q = (query ?? '').trim() || 'keywords:dsh-plugin'
        ...
        const items = (Array.isArray(json.objects) ? json.objects : [])
          .map((o) => {
            ...
            return {
              ...
              author: (p.publisher && p.publisher.username) || (p.author && p.author.name) || '',
              repository: (p.links && p.links.repository) || null,
  ...
  /** 安装一个 npm 包并加入 bundle 组合层（dsh plugin add 的等价物）。 */
  install(name) {
    const run = async () => {
      ...
      if (installedNames().has(name)) return { accepted: false, message: '已安装：' + name }
      const result = await this.runPnpm(['add', name + '@latest'])
      if (!result.ok) return { accepted: false, message: '安装失败' + summarize(result.output) }
      let reconciled = false
      const pkg = readInstalledPackage(name)
      if (pkg && pkg.dsh && pkg.dsh.bundle && pkg.dsh.bundle.patch) {
        const manifest = readProfileManifest()
      ...
      return { accepted: true, message: '已安装 ' + name + (reconciled ? '（已加入组合层）' : '（未声明 dsh.bundle，作为普通依赖）') + '，重启 profile 生效。' + summarize(result.output) }
    }
    const queued = this.packageQueue.then(run, run)
    this.packageQueue = queued.then(() => {}, () => {})
    return queued
  ...
  uninstall(name) {
    const run = async () => {
      if (typeof name !== 'string' || !name) return { accepted: false, message: '包名不能为空' }
      if (!PKG_NAME_RE.test(name)) return { accepted: false, message: '非法包名' }
      if (SYSTEM_MODULES.has(name) || name.startsWith('@deepseek-ai/') || name === 'dsh-plugin-manager') {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'GET' && pathname === '/plugin-manager/market') { const u = new URL(req.url || '/', 'http://x'); return sendJson(res, 200, await svc.market(u.searchParams.get('q') || '')) }
