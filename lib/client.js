window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function layoutGraph(nodes, edges, W, H) {
      ...
      const byModule = new Map(nodes.map((nd) => [nd.moduleName, nd]))
      const adj = new Map()
      for (const nd of nodes) adj.set(nd.moduleName, [])
      for (const e of edges) {
        if (adj.has(e.from)) adj.get(e.from).push(e.to)
        if (adj.has(e.to)) adj.get(e.to).push(e.from)
      }
      const iters = Math.max(80, Math.min(280, Math.round(n * 1.6)))
      for (let it = 0; it < iters; it++) {
        ...
        for (const e of edges) {
          const A = byModule.get(e.from)
          const B = byModule.get(e.to)
          if (!A || !B) continue
          const dx = B.x - A.x
          const dy = B.y - A.y
        ...
        for (const nd of nodes) {
          ...
          nd.y += (cy - nd.y) * 0.05
          if (nd.x < 20) nd.x = 20
          if (nd.x > W - 20) nd.x = W - 20
          if (nd.y < 20) nd.y = 20
          if (nd.y > H - 20) nd.y = H - 20
        }
      }
      // 碰撞消解：按节点实际宽度推开重叠
    ...
    function edgePath(A, B) {
      ...
      const dy = p.y2 - p.y1
      const d = Math.sqrt(dx * dx + dy * dy)
      if (d < 1) return 'M ' + p.x1 + ' ' + p.y1 + ' L ' + p.x2 + ' ' + p.y2
      const nx = dy / d
      const ny = -dx / d
      const bend = Math.min(26, d * 0.18)
      const cx = mx + nx * bend
      const cy = my + ny * bend
        ...
        h('div', { className: 'pmg-graph-panel' },
          ...
          h('svg', {
            ...
            h('defs', null,
              ...
              h('marker', { id: 'pmg-arrow', viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 9, markerHeight: 9, orient: 'auto-start-reverse' },
            ...
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              ...
              graph.edges.map((e) => {
                ...
                return h('path', { key: e.from + '\u2192' + e.to, d: edgePath(A, B), fill: 'none', className: cls, markerEnd: 'url(#pmg-arrow)' })
              }),
              nodePos.map((nd) => {
                let cls = 'pmg-gnode'
