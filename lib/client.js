window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.pmg-graph-edge{stroke:var(--dsw-alias-border-l1);stroke-width:1.3}',
      ...
      '.pmg-gnode-rect.official{fill:color-mix(in srgb,var(--dsw-alias-brand-primary) 12%,var(--dsw-alias-bg-layer-1));stroke:color-mix(in srgb,var(--dsw-alias-brand-primary) 55%,transparent)}',
      '.pmg-gnode-name{font-size:12px;font-weight:600;fill:var(--dsw-alias-label-primary)}',
      '.pmg-gnode-badge{font-size:10px;font-weight:600;fill:var(--dsw-alias-label-secondary)}',
      '.pmg-gnode-badge.official{fill:var(--dsw-alias-brand-primary)}',
    ...
    function DependencyGraph(props) {
      ...
      const fit = useCallback(() => {
        ...
        for (const nd of ns) {
          minX = Math.min(minX, nd.x - GNODE_W / 2); maxX = Math.max(maxX, nd.x + GNODE_W / 2)
          ...
          h('svg', {
            ...
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              ...
              laidOut.nodes.map((nd) => h('g', {
                ...
                h('rect', { x: -GNODE_W / 2, y: -GNODE_H / 2, width: GNODE_W, height: GNODE_H, rx: 10, className: 'pmg-gnode-rect ' + nd.source }),
                h('text', { y: -4, textAnchor: 'middle', className: 'pmg-gnode-name' }, truncateName(nd.name, 9)),
                h('text', { y: 13, textAnchor: 'middle', className: 'pmg-gnode-badge ' + nd.source }, nd.source === 'official' ? '官方' : '外部'),
              )),
            ),
          ),
          hover ? h('div', { className: 'pmg-graph-tip', style: { left: hover.x + 14, top: hover.y + 14 } },
            h('div', { className: 'pmg-graph-tip-name' }, hover.node.name),
            h('div', { className: 'pmg-graph-tip-mod' }, hover.node.moduleName),
            h('div', { className: 'pmg-graph-tip-line' }, (hover.node.source === 'official' ? '官方插件' : '外部插件') + (hover.node.system ? ' · 核心' : '')),
