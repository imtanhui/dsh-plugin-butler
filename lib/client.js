window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.pmg-gdot.official{background:var(--dsw-alias-brand-primary)}',
      '.pmg-gdot.external{background:var(--dsw-alias-label-secondary)}',
      ...
      '.pmg-gnode{cursor:pointer}',
      '.pmg-gnode-rect{fill:var(--dsw-alias-bg-layer-2);stroke:var(--dsw-alias-border-l2);stroke-width:1}',
      '.pmg-gnode-rect.official{fill:color-mix(in srgb,var(--dsw-alias-brand-primary) 12%,var(--dsw-alias-bg-layer-1));stroke:color-mix(in srgb,var(--dsw-alias-brand-primary) 50%,transparent)}',
      '.pmg-gnode-name{font-size:11px;font-weight:600;fill:var(--dsw-alias-label-primary)}',
      '.pmg-gnode-dot.official{fill:var(--dsw-alias-brand-primary)}',
      '.pmg-gnode-dot.external{fill:var(--dsw-alias-label-secondary)}',
    ...
    function nodeWidth(s) {
      return Math.max(52, Math.round(24 + s.length * 6.6))
    }
    ...
    function buildGraph(data, filter) {
      ...
      for (const e of entries) {
        for (const s of e.injects) {
          const prov = providers[s]
          if (!prov || !byModule.has(prov) || prov === e.moduleName) continue
          const key = e.moduleName + '\u2192' + prov
          if (seen.has(key)) continue
          seen.add(key)
          edges.push({ from: e.moduleName, to: prov })
        }
      }
      return { nodes, edges }
    }
    ...
    function DependencyGraph(props) {
      ...
      const nodeByModule = useMemo(() => {
        const m = new Map()
        for (const nd of laidOut.nodes) m.set(nd.moduleName, nd)
        return m
      }, [laidOut])

      const fit = useCallback(() => {
        const el = svgRef.current
        const ns = laidOut.nodes
        ...
        h('div', { className: 'pmg-graph-panel' },
          h('div', { className: 'pmg-graph-head' },
            ...
            h('div', { className: 'pmg-graph-btns' },
              ...
              h('button', { type: 'button', className: 'pmg-btn', onClick: fit }, '适应'),
            ),
            h('button', { type: 'button', className: 'pmg-modal-close', onClick: props.onClose, 'aria-label': '关闭' }, '\u00d7'),
          ),
          h('svg', {
            ref: svgRef, className: 'pmg-graph-svg',
            onWheel: onWheel, onMouseDown: onPanStart, onMouseMove: onPanMove, onMouseUp: onPanEnd, onMouseLeave: onPanEnd,
          },
            h('defs', null,
            ...
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              ...
              laidOut.nodes.map((nd) => h('g', {
                ...
                h('circle', { cx: -nd.w / 2 + 13, cy: 0, r: 4, className: 'pmg-gnode-dot ' + nd.source }),
                h('text', { x: -nd.w / 2 + 23, y: 0, textAnchor: 'start', dominantBaseline: 'central', className: 'pmg-gnode-name' }, nd.proj),
              )),
            ),
          ),
