window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.pmg-graph-edge{stroke:var(--dsw-alias-border-l1);stroke-width:1.1;opacity:.45}',
      '.pmg-graph-arrow{fill:var(--dsw-alias-border-l1)}',
      ...
      return h('div', { className: 'pmg-graph-overlay', onClick: (e) => { if (e.target === e.currentTarget) props.onClose() } },
        h('div', { className: 'pmg-graph-panel' },
          ...
          h('svg', {
            ...
            h('defs', null,
              h('marker', { id: 'pmg-arrow', viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 7, markerHeight: 7, orient: 'auto-start-reverse' },
            ...
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              graph.edges.map((e) => {
                ...
                if (!A || !B) return null
                return h('line', { key: e.from + '\u2192' + e.to, x1: A.x, y1: A.y, x2: B.x, y2: B.y, className: 'pmg-graph-edge', markerEnd: 'url(#pmg-arrow)' })
              }),
              nodePos.map((nd) => h('g', {
