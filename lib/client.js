window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    const { createElement: h, useState, useEffect, useCallback, useMemo, useRef, Fragment } = require('react')
    ...
    const css = [
      ...
      '.pmg-graph-overlay{position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;padding:20px}',
      '.pmg-graph-panel{width:100%;height:100%;max-width:1400px;max-height:900px;display:flex;flex-direction:column;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:14px;overflow:hidden}',
      '.pmg-graph-head{display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid var(--dsw-alias-border-l2);flex-wrap:wrap}',
      '.pmg-graph-title{font-weight:650;font-size:15px}',
      '.pmg-graph-legend{display:flex;align-items:center;gap:12px;margin-left:auto}',
      '.pmg-glegend-item{display:inline-flex;align-items:center;gap:5px;font-size:11px;color:var(--dsw-alias-label-secondary)}',
      '.pmg-gdot{width:10px;height:10px;border-radius:3px;display:inline-block}',
      '.pmg-gdot.official{background:#2563eb}',
      '.pmg-gdot.external{background:#1f2937}',
      '.pmg-graph-btns{display:flex;gap:6px}',
      '.pmg-graph-svg{flex:1;width:100%;cursor:grab}',
      '.pmg-graph-svg:active{cursor:grabbing}',
      '.pmg-graph-edge{stroke:#9aa9bb;stroke-width:1.2;opacity:.22}',
      '.pmg-graph-arrow{fill:#64748b;opacity:.55}',
      '.pmg-graph-arrow-active{fill:#2563eb}',
      '.pmg-graph-dot{fill:var(--dsw-alias-border-l2);opacity:.5}',
      '.pmg-graph-edge{transition:opacity .18s,stroke-width .18s,stroke .18s}',
      '.pmg-graph-edge.dim{opacity:.05}',
      '.pmg-graph-edge.active{stroke:#2563eb;stroke-width:2.2;opacity:1}',
      '.pmg-gnode{transition:opacity .18s}',
      '.pmg-gnode.dim{opacity:.28}',
      '.pmg-gnode{cursor:grab}',
      '.pmg-gnode:active{cursor:grabbing}',
      '.pmg-gnode-rect{fill:var(--dsw-alias-bg-layer-2);stroke:var(--dsw-alias-border-l2);stroke-width:1}',
      '.pmg-gnode-rect.official{fill:color-mix(in srgb,var(--dsw-alias-brand-primary) 10%,var(--dsw-alias-bg-layer-1));stroke:color-mix(in srgb,var(--dsw-alias-brand-primary) 45%,transparent)}',
      '.pmg-gnode-rect.external{fill:var(--dsw-alias-bg-layer-2);stroke:var(--dsw-alias-border-l2)}',
      '.pmg-gnode-name{font-size:11px;font-weight:600;fill:var(--dsw-alias-label-primary)}',
      '.pmg-gnode-dot.official{fill:#2563eb}',
      '.pmg-gnode-dot.external{fill:#1f2937}',
      '.pmg-graph-tip{position:fixed;z-index:1100;max-width:300px;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;box-shadow:0 8px 24px -8px rgba(0,0,0,.35);padding:10px 12px;pointer-events:none;font-size:12px;line-height:18px}',
      '.pmg-graph-tip-name{font-weight:650;color:var(--dsw-alias-label-primary)}',
      '.pmg-graph-tip-mod{color:var(--dsw-alias-label-secondary);font-size:11px;word-break:break-all}',
      '.pmg-graph-tip-desc{color:var(--dsw-alias-label-secondary);margin-top:4px}',
      '.pmg-graph-tip-line{color:var(--dsw-alias-label-secondary);margin-top:2px}',
    ].join('\n')
          ...
          showDeps ? h('div', { className: 'pmg-deps' },
            h('div', { className: 'pmg-deps-line' },
              ...
              entry.injects && entry.injects.length
                ? entry.injects.map((it) => h('span', { key: it.service, className: 'pmg-dep-tag' }, it.service, it.provider ? h('span', { className: 'src' }, '\u2190 ' + shortName(it.provider)) : null))
    ...
    function projName(n) {
      return n && n.startsWith('@') ? n.slice(n.indexOf('/') + 1) : (n || '')
    }

    function nodeWidth(s) {
      return Math.max(58, Math.round(34 + s.length * 6.6))
    }

    function buildGraph(data, filter) {
      const entries = (data.entries || []).filter((e) => filter === 'all' || e.source === 'external')
      const byModule = new Map()
      for (const e of entries) byModule.set(e.moduleName, e)
      const nodes = entries.map((e) => ({
        id: e.entryId,
        moduleName: e.moduleName,
        proj: truncateName(projName(e.moduleName), 22),
        w: 0,
        name: e.displayName,
        source: e.source,
        system: e.system,
      ...
      for (const nd of nodes) nd.w = nodeWidth(nd.proj)
      const edges = []
      const seen = new Set()
      for (const e of (data.edges || [])) {
        if (!byModule.has(e.from) || !byModule.has(e.to)) continue
    ...
    function layoutGraph(nodes, edges, W, H) {
      // 从左到右的思维导图式布局：最左列 = 核心（被依赖最多），向右逐列递减。
      const incoming = new Map()
      const outgoing = new Map()
      for (const nd of nodes) { incoming.set(nd.moduleName, 0); outgoing.set(nd.moduleName, new Set()) }
      for (const e of edges) {
        if (incoming.has(e.to)) incoming.set(e.to, incoming.get(e.to) + 1)
        if (outgoing.has(e.from)) outgoing.get(e.from).add(e.to)
      }
      const colOf = (fi) => (fi >= 16 ? 0 : fi >= 8 ? 1 : fi >= 4 ? 2 : fi >= 2 ? 3 : fi >= 1 ? 4 : -1)
      let leafIdx = 0
      const getCol = (m) => {
        const fi = incoming.get(m) || 0
        if (fi > 0) return colOf(fi)
        leafIdx++
        return 5 + ((leafIdx - 1) % 3)
      }
      const COLS = 8
      const byCol = new Map()
      for (const nd of nodes) {
        const c = getCol(nd.moduleName)
        if (!byCol.has(c)) byCol.set(c, [])
        byCol.get(c).push(nd)
      }

      // 逐列放置：同列内用重心法排序（靠近其依赖的垂直位置），减少连线交叉。
      const colGap = 230
      const marginX = 90
      const rowGap = 16
      const yOf = new Map()
      for (let c = 0; c < COLS; c++) {
        ...
        let sorted
        if (c === 0) {
          sorted = list.slice().sort((a, b) => a.moduleName.localeCompare(b.moduleName))
        } else {
          const bary = (nd) => {
            let best = null
            let bestFi = -1
            let sum = 0
            let cnt = 0
            for (const dep of outgoing.get(nd.moduleName)) {
              if (yOf.has(dep)) {
                sum += yOf.get(dep)
                cnt++
                const fi = incoming.get(dep)
                if (fi > bestFi) { bestFi = fi; best = yOf.get(dep) }
              }
            }
            return best !== null ? best : (cnt ? sum / cnt : null)
          }
          const withB = []
          const without = []
          for (const nd of list) {
            const b = bary(nd)
            if (b !== null) withB.push({ nd, b })
            else without.push(nd)
          }
          withB.sort((a, b) => a.b - b.b)
          sorted = withB.map((x) => x.nd).concat(without)
        }
        const x = marginX + c * colGap
        const colH = sorted.length * (GNODE_H + rowGap)
        let y = Math.max(40, (H - colH) / 2)
    ...
    function edgeEndpoints(A, B) {
      ...
      if (d < 1) return { x1: A.x, y1: A.y, x2: B.x, y2: B.y }
      const ux = dx / d
      const uy = dy / d
      const tOf = (nd) => {
        const rx = nd.w / 2
    ...
    function DependencyGraph(props) {
      ...
      const graph = useMemo(() => buildGraph(props.data, filter), [props.data, filter])
      const [nodePos, setNodePos] = useState([])

      const fitToNodes = useCallback((ns) => {
        const el = svgRef.current
        if (!el || !ns.length) return
        let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity
        for (const nd of ns) {
          minX = Math.min(minX, nd.x - nd.w / 2); maxX = Math.max(maxX, nd.x + nd.w / 2)
          minY = Math.min(minY, nd.y - GNODE_H / 2); maxY = Math.max(maxY, nd.y + GNODE_H / 2)
        }
        const w = Math.max(1, maxX - minX + 80)
        const h = Math.max(1, maxY - minY + 80)
        const vw = el.clientWidth || 800
        const vh = el.clientHeight || 600
        const scale = Math.min(1.5, Math.max(0.05, Math.min(vw / w, vh / h)))
      ...
      useEffect(() => {
        const ns = graph.nodes.map((nd) => ({ ...nd }))
        const es = graph.edges.map((e) => ({ ...e }))
        layoutGraph(ns, es, 2100, 1800)
        setNodePos(ns)
        fitToNodes(ns)
      }, [graph, fitToNodes])

      const posByModule = useMemo(() => {
        const m = new Map()
        for (const nd of nodePos) m.set(nd.moduleName, nd)
        return m
      }, [nodePos])
      ...
      const hoverModule = hover ? hover.node.moduleName : null
      const hoverNeighbors = hoverModule ? (adjacency.get(hoverModule) || null) : null
      ...
      const onWheel = (e) => {
        ...
        setView((v) => {
          ...
          const tx = mx - ((mx - v.tx) / v.scale) * scale
          const ty = my - ((my - v.ty) / v.scale) * scale
          return { tx, ty, scale }
        })
      }
      const onPanStart = (e) => {
        if (e.target && e.target.closest && e.target.closest('.pmg-gnode')) return
        dragRef.current = { x: e.clientX, y: e.clientY, tx: view.tx, ty: view.ty }
      }
      const onMove = (e) => {
        const nd = nodeDragRef.current
        if (nd) {
          const dx = (e.clientX - nd.x) / view.scale
          const dy = (e.clientY - nd.y) / view.scale
          nd.x = e.clientX
          nd.y = e.clientY
          setNodePos((prev) => prev.map((p) => (p.moduleName === nd.moduleName ? { ...p, x: p.x + dx, y: p.y + dy } : p)))
          return
        }
      ...
      const onNodeDown = (e, moduleName) => {
        e.stopPropagation()
        nodeDragRef.current = { moduleName, x: e.clientX, y: e.clientY }
      }
      const onEnd = () => { dragRef.current = null; nodeDragRef.current = null }
      const zoomBy = (factor) => {
        setView((v) => {
          const scale = Math.min(4, Math.max(0.08, v.scale * factor))
          const el = svgRef.current
          const mx = el ? el.clientWidth / 2 : 400
          const my = el ? el.clientHeight / 2 : 300
          const tx = mx - ((mx - v.tx) / v.scale) * scale
          const ty = my - ((my - v.ty) / v.scale) * scale
          return { tx, ty, scale }
        })
      ...
      return h('div', { className: 'pmg-graph-overlay', onClick: (e) => { if (e.target === e.currentTarget) props.onClose() } },
        h('div', { className: 'pmg-graph-panel' },
          h('div', { className: 'pmg-graph-head' },
            h('div', { className: 'pmg-graph-title' }, '依赖关系图'),
            h('span', { className: 'pmg-sub' }, graph.nodes.length + ' 个插件 · ' + graph.edges.length + ' 条依赖'),
            h('select', { className: 'pmg-select', value: filter, onChange: (e) => setFilter(e.currentTarget.value) },
              h('option', { value: 'all' }, '全部插件'),
              h('option', { value: 'external' }, '仅外部'),
            ),
            h('div', { className: 'pmg-graph-legend' },
              h('span', { className: 'pmg-glegend-item' }, h('i', { className: 'pmg-gdot official' }), '官方'),
              h('span', { className: 'pmg-glegend-item' }, h('i', { className: 'pmg-gdot external' }), '外部'),
            ),
            h('div', { className: 'pmg-graph-btns' },
              h('button', { type: 'button', className: 'pmg-btn', onClick: () => zoomBy(1.25) }, '+'),
              h('button', { type: 'button', className: 'pmg-btn', onClick: () => zoomBy(0.8) }, '\u2212'),
              h('button', { type: 'button', className: 'pmg-btn', onClick: () => fitToNodes(nodePos) }, '适应'),
            ),
            h('button', { type: 'button', className: 'pmg-modal-close', onClick: props.onClose, 'aria-label': '关闭' }, '\u00d7'),
          ),
          h('svg', {
            ref: svgRef, className: 'pmg-graph-svg',
            onWheel: onWheel, onMouseDown: onPanStart, onMouseMove: onMove, onMouseUp: onEnd, onMouseLeave: onEnd,
          },
            h('defs', null,
              h('pattern', { id: 'pmg-dots', width: 32, height: 32, patternUnits: 'userSpaceOnUse' },
                h('circle', { cx: 2, cy: 2, r: 1.1, className: 'pmg-graph-dot' }),
              ),
              h('marker', { id: 'pmg-arrow', viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 7.5, markerHeight: 7.5, orient: 'auto-start-reverse' },
                h('path', { d: 'M 0 0 L 10 5 L 0 10 z', className: 'pmg-graph-arrow' }),
              ),
              h('marker', { id: 'pmg-arrow-active', viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 7.5, markerHeight: 7.5, orient: 'auto-start-reverse' },
                h('path', { d: 'M 0 0 L 10 5 L 0 10 z', className: 'pmg-graph-arrow-active' }),
              ),
            ),
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              h('rect', { x: -5000, y: -4000, width: 10000, height: 8000, fill: 'url(#pmg-dots)' }),
              graph.edges.map((e) => {
                const A = posByModule.get(e.from)
                const B = posByModule.get(e.to)
                if (!A || !B) return null
                let cls = 'pmg-graph-edge'
                let active = false
                if (hoverModule) {
                  active = e.from === hoverModule || e.to === hoverModule
                  cls += active ? ' active' : ' dim'
                }
                const p = edgeEndpoints(A, B)
                return h('line', { key: e.from + '\u2192' + e.to, x1: p.x1, y1: p.y1, x2: p.x2, y2: p.y2, className: cls, markerEnd: active ? 'url(#pmg-arrow-active)' : 'url(#pmg-arrow)' })
              }),
              nodePos.map((nd) => {
                let cls = 'pmg-gnode'
                if (hoverModule && nd.moduleName !== hoverModule) cls += hoverNeighbors && hoverNeighbors.has(nd.moduleName) ? ' neighbor' : ' dim'
                return h('g', {
                  key: nd.id, className: cls, transform: 'translate(' + nd.x + ',' + nd.y + ')',
                  onMouseDown: (e) => onNodeDown(e, nd.moduleName),
                  onMouseEnter: (e) => setHover({ node: nd, x: e.clientX, y: e.clientY }),
                  onMouseMove: (e) => setHover((hh) => (hh ? { ...hh, x: e.clientX, y: e.clientY } : hh)),
                  onMouseLeave: () => setHover(null),
                },
                  h('rect', { x: -nd.w / 2, y: -GNODE_H / 2, width: nd.w, height: GNODE_H, rx: GNODE_H / 2, className: 'pmg-gnode-rect ' + nd.source }),
                  h('circle', { cx: -nd.w / 2 + 10, cy: 0, r: 4, className: 'pmg-gnode-dot ' + nd.source }),
                  h('text', { x: -nd.w / 2 + 19, y: 0, textAnchor: 'start', dominantBaseline: 'central', className: 'pmg-gnode-name' }, nd.proj),
                )
              }),
            ),
          ),
          hover ? h('div', { className: 'pmg-graph-tip', style: { left: hover.x + 14, top: hover.y + 14 } },
            h('div', { className: 'pmg-graph-tip-name' }, hover.node.proj + ' · ' + (hover.node.source === 'official' ? '官方' : '外部')),
            h('div', { className: 'pmg-graph-tip-mod' }, hover.node.moduleName),
            hover.node.name && hover.node.name !== hover.node.proj ? h('div', { className: 'pmg-graph-tip-desc' }, hover.node.name) : null,
            hover.node.description ? h('div', { className: 'pmg-graph-tip-desc' }, hover.node.description) : null,
            h('div', { className: 'pmg-graph-tip-line' }, '注入 ' + hover.node.injects.length + ' 个服务 · 被 ' + hover.node.dependents.length + ' 个插件依赖'),
          ) : null,
        ),
      )
    }
    ...
    function PluginManagerTab(props) {
      ...
      return h('div', { className: 'pmg' },
        ...
        h('div', { className: 'pmg-toolbar' },
          ...
          h('button', { type: 'button', className: 'pmg-btn', onClick: () => setShowGraph(true) }, '依赖图'),
        ),
        ...
        showGraph ? h(DependencyGraph, { data, onClose: () => setShowGraph(false) }) : null,
      )
    }
