window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function layoutGraph(nodes, edges, W, H) {
      // 环序：按被依赖数（fan-in）分层——中心 = 最被依赖的核心，向外逐层递减。
      const incoming = new Map()
      for (const nd of nodes) incoming.set(nd.moduleName, 0)
      for (const e of edges) if (incoming.has(e.to)) incoming.set(e.to, incoming.get(e.to) + 1)
      const innerRingOf = (fi) => (fi >= 16 ? 0 : fi >= 8 ? 1 : fi >= 4 ? 2 : fi >= 2 ? 3 : fi >= 1 ? 4 : -1)
      let leafIdx = 0
      const ringOf = (m) => {
        const fi = incoming.get(m) || 0
        if (fi > 0) return innerRingOf(fi)
        leafIdx++
      ...
      for (const nd of nodes) {
        const ring = ringOf(nd.moduleName)
        if (!byRing.has(ring)) byRing.set(ring, [])
        byRing.get(ring).push(nd)
      }

      // 半径：逐环单调递增，按环上节点总宽保证同环不重叠。
      const cx = W / 2
      const cy = H / 2
      const radii = new Map()
      let prevR = 60
      for (let ring = 0; ring <= RING_MAX; ring++) {
        ...
        let widthSum = 0
        for (const nd of list) widthSum += nd.w + 18
        const needed = widthSum / (2 * Math.PI)
        const r = Math.max(prevR + (list.length ? 130 : 0), needed)
        radii.set(ring, Math.max(r, prevR))
        if (list.length) prevR = r
      }

      // 放置：每环均分角度，相邻环错开半个间隔。
      for (const [ring, list] of byRing) {
        const r = radii.get(ring)
        const offset = (ring % 2) * (Math.PI / Math.max(1, list.length))
        list.forEach((nd, i) => {
          const ang = (i / list.length) * 2 * Math.PI + offset
