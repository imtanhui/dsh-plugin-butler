window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.pmg-graph-edge{stroke:#94a3b8;stroke-width:1.6;opacity:.9}',
      '.pmg-graph-arrow{fill:#475569}',
      '.pmg-graph-dot{fill:var(--dsw-alias-border-l2);opacity:.5}',
      '.pmg-graph-edge{transition:opacity .18s,stroke-width .18s,stroke .18s}',
      '.pmg-graph-edge.dim{opacity:.13}',
      '.pmg-graph-edge.active{stroke:#2563eb;stroke-width:2.4;opacity:1}',
        ...
        if (c === 0) {
          ...
          const bary = (nd) => {
            ...
            for (const dep of outgoing.get(nd.moduleName)) {
              if (yOf.has(dep)) { sum += yOf.get(dep); cnt++ }
            }
          ...
          h('svg', {
            ...
            h('g', { transform: 'translate(' + view.tx + ',' + view.ty + ') scale(' + view.scale + ')' },
              ...
              graph.edges.map((e) => {
                ...
                if (hoverModule) cls += (e.from === hoverModule || e.to === hoverModule) ? ' active' : ' dim'
                const p = edgeEndpoints(A, B)
                return h('line', { key: e.from + '\u2192' + e.to, x1: p.x1, y1: p.y1, x2: p.x2, y2: p.y2, className: cls, markerEnd: 'url(#pmg-arrow)' })
              }),
              nodePos.map((nd) => {
                let cls = 'pmg-gnode'
